...
export default function IdeaHoverPopover({ show, anchorRect, insight, idea }) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={style}
        className="backdrop-blur-md bg-white/85 border border-gray-200 shadow-2xl rounded-xl w-[300px]"
      >
        <div className="p-3">
          ...
          {idea && (
            <div className="mb-2 grid grid-cols-2 gap-2 text-[11px] text-[#2D3748]/80">
              <div className="truncate">
                <span className="text-[#061F59] font-medium">Category: </span>
                {idea.category || "—"}
              </div>
              <div className="truncate text-right">
                <span className="text-[#061F59] font-medium">Invest: </span>
                {idea.investmentRange || idea.investment || "—"}
              </div>
              <div className="truncate">
                <span className="text-[#061F59] font-medium">Rating: </span>
                {typeof idea.rating === "number"
                  ? idea.rating.toFixed(1)
                  : (idea.rating || 4.5).toFixed?.(1) || "4.5"}
              </div>
              <div className="truncate text-right">
                <span className="text-[#061F59] font-medium">TTM: </span>
                {idea.timeToStart || idea.timeToMarket || "—"}
              </div>
            </div>
          )}
          ...
          <div className="relative w-full overflow-hidden rounded-md bg-gray-50 border border-gray-200">
            ...
            >
              <defs>
                <linearGradient id="sparkGradIdea" x1="0" x2="0" y1="0" y2="1">
          ...
          {idea?.title && (
            <div className="mt-2 flex items-center justify-between">
              <div className="text-[11px] font-medium text-[#061F59] truncate pr-2">
                {idea.title}
              </div>
              <div className="text-[10px] px-2 py-1 rounded-full bg-[#FDCC29]/20 text-[#061F59] border border-[#FDCC29]/40">
                View
              </div>
            </div>
          )}
